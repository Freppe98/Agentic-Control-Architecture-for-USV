...
| `CommsPill(state,{full})` | `components/CommsPill.js` | built | comms state string | `full` → dot + word (`commpill`); else short `CONN/PART/DISC/UNK`. The **comms axis** — never used for health. | Map, Fleet, Vehicle, Video, Pilot, Agent |
...
| `Bar(pct,color)` | `lib/ui.js` (`bar`) | built | generic progress | coverage, disk, sector, confidence. | Mission, Vehicle, Agent |
| `HealthBadge(sev,condition)` | `components/HealthBadge.js` | built | `sev` OK/CAUTION/WARN, `condition` | Round severity dot + **named condition** ("Battery low"), never "Caution" alone. The **health axis**. | Fleet, Vehicle |
| `AuthoritySeg(authVal)` | `components/AuthoritySeg.js` | built | `authVal` "OPERATOR"\|"LOCAL_AGENT"\|null | Compact RC / Operator / Local Agent segmented indicator. Operator/Local Agent toggle from the live control-authority fetch; RC always renders "ready" — a stated architecture invariant (RC keeps hardware override regardless of authority), not a telemetry field. | Map inspector, Vehicle (Vehicle State, Control) |
| `StatusBadges(vehicle,authVal)` | `components/StatusBadges.js` | built | vehicle, authVal | Compact operator-glance strip: armed, mode, `AuthoritySeg`, comms, mission state. Small pills only — Live Fleet stays operational, not diagnostic. | Map inspector |
...
| `VehicleDock(list,sel,onSelect)` | `components/VehicleDock.js` | built | vehicles, selected id | Left roster; activity verb primary, lane/last-contact sub, battery. Assigned/Depot groups. | Map, Vehicle, Agent, Pilot |
| `Table(cols,rows,{sort})` | `components/Table.js` | built | columns, rows | Sortable dense table; sticky header, severity/selection rows. | Fleet, Vehicle matrix, Events |
...
| `DecisionTrace(trace)` | `components/DecisionTrace.js` | planned | state-machine nodes | Vertical trace; comms nodes = comms colors, behavior nodes = accent, future = dashed. **NO TELEM until agent emits it.** | Agent |
| `ConstraintList(constraints)` | `components/ConstraintList.js` | planned | met/unmet inputs | ✓/✗ decision inputs. **NO TELEM.** | Agent |
...
| `CommandChip(state)` | `components/CommandChip.js` | planned | Pending/Executing/Finished/Failed | Command lifecycle. **Needs command API.** | Mission, Map, Agent |
...
comms `Timeline` (needs comms-state transition log) · all `Agent` reasoning fields · `rssi`/`latency` · Vehicle `Temperatures`, pack voltage/current, compass cal · named **mission scope** & mission ETA/remaining · command lifecycle.
