...
- Endpoints: `POST /api/commands` · `GET /api/commands/pending/{id}` (claims QUEUED→SENT, redelivers SENT at-least-once) · `POST /api/commands/{command_id}/result` (idempotent) · `GET /api/commands/{id}` (queue + history + active) · `GET /api/commands/history/{id}` (terminal only) · `GET /api/command/{command_id}` (**singular**: one command by uuid).
- **Reading ONE command by its uuid** — use the singular `GET /api/command/{command_id}`, which resolves the uuid against `commands_by_id` and returns `{ok, command}` (verification + lifecycle refreshed) or a clean JSON 404. Do **not** hand a uuid to the plural per-vehicle `GET /api/commands/{vehicle_id}`: that path arg is a VEHICLE id, so a uuid parses to `vehicle_id: -1` and returns an empty list — a silent false negative, not an error. There is intentionally no `GET /api/commands` (only `POST`), so a bare GET there is `405 Method Not Allowed`; the singular route is the by-id read path.
- **Rover mode representation** — a command's verified mode is compared *canonically* (`normalize_rover_mode`): a numeric ArduPilot custom_mode (`11`), its string (`"11"`) and the name (`"RTL"`) all resolve to the same canonical name, so an EXECUTED+verified RTL whose read-back is `11` is **VERIFIED, not FAILED**. `verification.observed` is the canonical name; `verification.observed_raw` / `command.observed_raw` retain Scout's raw value. An unrecognised observed representation is `rtl_result: "unverified"` (EXECUTED_UNVERIFIED — respects Scout's authority, refuses a silent green), never a fabricated vehicle rejection.
